...
/**
 * @brief Initialize serial output and enable the speaker DAC channel.
 *
...
/*---------------------------------------------------------------
 * Waveform data
 * One 256-sample cycle maps a sine wave to the DAC's 8-bit range.
 *--------------------------------------------------------------*/
...
/**
 * @brief Generate a sine-wave tone through the ESP32 DAC.
 *
 * Each waveform cycle contains 256 samples. The requested frequency therefore
 * determines the sample rate and the delay between consecutive DAC writes.
 * This function is called by loop() whenever a tone should be played.
 *
...
void playSineWave(int frequency, int durationMs) {
  ...
  // Returning to the waveform midpoint removes the DC step after playback.
