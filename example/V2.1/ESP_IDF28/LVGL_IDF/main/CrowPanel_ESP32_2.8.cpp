...
// Keep the sensor on ESP32's second I2C controller so it does not contend
// with framework services which may initialize the default Wire bus.
TwoWire dhtWire(1);
DHT20 dht20(&dhtWire);
...
extern "C" void app_main() {
  ...
  bool dht_ready = dhtWire.begin(22, 21, 100000);
  if (!dht_ready) {
    Serial.println("I2C bus init failed");
  }
  delay(100);
  if (dht_ready && dht20.begin() != 0) {
  ...
  while (1) {
    ...
    if (now - last_sensor_ms >= 1000) {
      ...
      int temperature = 0;
      int humidity = 0;
      if (dht_ready && dht20.readTempHumidity(&temperature, &humidity) == 0) {
        char dht_buffer[8];
